...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      if (storedToken) {
        try {
          const response = await api.get('/api/core/profile/');
  ...
  const login = async (credentials) => {
    try {
      const response = await api.post('/api/core/login/', credentials);
  ...
  const register = async (userData) => {
    try {
      const response = await api.post('/api/core/register/', userData);
